...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobs($user_id)
    {
        ...
        $emergencyJobs = collect($jobs)->filter(function ($jobitem) {
            return $jobitem->immediate == 'yes';
        })->all();

        $noramlJobs = collect($jobs)->reject(function ($jobitem) {
            return $jobitem->immediate == 'yes';
    ...
    public function store($user, $data)
    {
        ...
        if ($user->user_type == env('CUSTOMER_ROLE_ID')) {
            ...
            if ($data['immediate'] == 'no') {
                $requiredFields = ['due_date', 'due_time', 'customer_phone_type', 'duration'];
            ...
            $certifiedMap = [
                'normal' => 'normal',
                'certified' => 'certified',
                'certified_in_law' => 'n_law',
                'certified_in_helth' => 'n_health',
            ...
            foreach ($data['job_for'] as $job) {
                if (array_key_exists($job, $genderMap)) {
                    $data['gender'] = $job;
                } else if (array_key_exists($job, $certifiedMap)) {
                    $data['certified'] = $certifiedMap[$job];
                }
            }
            ...
            $data['job_type'] = $jobTypeMap[$consumer_type] ?? null;
            ...
            if ($job->certified != null) {
                $certifiedValue = $certifiedMap[$job->certified];

                if (is_array($certifiedValue)) {
                    $data['job_for'] = array_merge($data['job_for'], $certifiedValue);
                } else {
                    $data['job_for'][] = $certifiedValue;
                }
            }
        } else {
            $response['status'] = 'fail';
    ...
    public function storeJobEmail($data)
    {
        ...
        if (isset($data['address'])) {
            $job->fill([
                'address' => ($data['address'] != '') ? $data['address'] : $job->user->userMeta->address,
                'instructions' => ($data['instructions'] != '') ? $data['instructions'] : $job->user->userMeta->instructions,
                'town' => ($data['town'] != '') ? $data['town'] : $job->user->userMeta->city,
            ]);
        }
        $job->save();
    ...
    public function jobToData($job)
    {
        ...
        $customerType = $job->user->userMeta->customer_type;
        ...
        if (isset($certifiedMap[$job->certified])) {
            $jobFor = array_merge($jobFor, $certifiedMap[$job->certified]);
    ...
    public function jobEnd($post_data = array())
    {
        ...
        $completedDate = date_create();
        $interval = $completedDate->diff($dueDate)->format('%h tim %i min %s sek');
        ...
        $translator = $job->translatorJobRel->where('completed_at', Null)->where('cancel_at', Null)->first()->user()->first();
        ...
        // Update translator job details
        $translatorJob = $job->translatorJobRel->where('completed_at', Null)->where('cancel_at', Null)->first();
        $translatorJob->completed_at = date('Y-m-d H:i:s');
        $translatorJob->completed_by = $post_data['userid'];
        $translatorJob->save();

        // Fire session ended event
        $eventUserId = ($post_data['userid'] == $job->user_id) ? $translatorJob->user_id : $job->user_id;
        Event::fire(new SessionEnded($job, $eventUserId));
    }
    ...
    public function sendNotificationTranslator($job, $data = [], $exclude_user_id)
    {
        ...
        foreach ($users as $oneUser) {
            ...
            $job_for_translator = $jobs->firstWhere('id', $job->id);

            if (!$job_for_translator) {
                continue;
            }

            if ($job_for_translator->isAssignedToTranslator($oneUser->id)) {
                $job_checker = Job::checkParticularJob($oneUser->id, $job_for_translator);
                if ($job_checker === 'userCanNotAcceptJob') {
                    continue;
                }

                $translator_list = $this->isNeedToDelayPush($oneUser->id) ? $delpay_translator_array : $translator_array;
                $translator_list[] = $oneUser;
            }
        }
    ...
    public function updateJob($id, $data, $cuser)
    {
        ...
        $log_data = [];
        $current_translator_null = $job->translatorJobRel->where('cancel_at', Null)->first();
        $current_translator_not_null = $job->translatorJobRel->where('completed_at', '!=', Null)->first();
        if ($current_translator_null) {
            $changeTranslator = $this->changeTranslator($current_translator_null, $data, $job);
            if ($changeTranslator['translatorChanged']) $log_data[] = $changeTranslator['log_data'];
        } elseif ($current_translator_not_null) {
            // Use the first translator with a completed job if no active translator is found
            $changeTranslator = $this->changeTranslator($current_translator_not_null, $data, $job);
            if ($changeTranslator['translatorChanged']) $log_data[] = $changeTranslator['log_data'];
        }

        if ($changeDue = $this->changeDue($job->due, $data['due'])) {
            $job->due = $data['due'];
        ...
        if ($changeStatus = $this->changeStatus($job, $data, $changeTranslator['translatorChanged'])) {
        ...
        if ($job->from_language_id != $old_lang) {
    ...
    private function changeCompletedStatus($job, $data)
    {
        ...
        $job->fill([
            'status' => $data['status'],
            'admin_comments' => $data['admin_comments'] ?? $job->admin_comments,
        ])->save();
    ...
    {
        $logFilePath = storage_path('logs/cron/laravel-' . date('Y-m-d') . '.log');
        $this->logger->pushHandler(new StreamHandler($logFilePath, Logger::DEBUG));
        $this->logger->pushHandler(new FirePHPHandler());

        $notificationType = 'session_start_remind';
        $dueExploded = explode(' ', $due);
        $isPhysical = $job->customer_physical_type == 'yes';
        $town = $job->town;
        $msgText = sprintf(
            'Detta är en påminnelse om att du har en %s-tolkning (%s) kl %s på %s som kommer att vara i %d min. Lycka till och kom ihåg att ge feedback efter utförd tolkning!',
            $language,
            $isPhysical ? "på plats i $town" : 'telefon',
            $dueExploded[1],
            $dueExploded[0],
            $duration
        );
        $data = compact('notificationType');
        $usersArray = [$user];

        if ($this->bookingRepository->isNeedToSendPush($user->id)) {
            $isDelayed = $this->bookingRepository->isNeedToDelayPush($user->id);
    ...
    private function changeTranslator($currentTranslator, $data, $job)
    {
        ...
        if ($currentTranslator || ($data['translator'] ?? 0) || $data['translator_email']) {
            $logData = [];
            $newTranslator = null;

            if ($currentTranslator && (($data['translator'] ?? $currentTranslator->user_id) != $currentTranslator->user_id || $data['translator_email'])) {
                $newTranslator = $currentTranslator->replicate();
                $newTranslator->user_id = $data['translator_email'] ? User::where('email', $data['translator_email'])->value('id') : $data['translator'];
                $newTranslator->push();
            ...
            } elseif (!$currentTranslator && ($data['translator'] ?? 0 || $data['translator_email'])) {
                $newTranslator = Translator::create([
                    'user_id' => $data['translator_email'] ? User::where('email', $data['translator_email'])->value('id') : $data['translator'],
                    'job_id' => $job->id,
                ]);
                $logData[] = [
                    'old_translator' => null,
    ...
    public function sendChangedTranslatorNotification($job, $current_translator, $new_translator)
    {
        ...
        if ($current_translator) {
            $old_user = $current_translator->user;
            $old_name = $old_user->name;
            $old_email = $old_user->email;

            $this->mailer->send($old_email, $old_name, $subject, 'emails.job-changed-translator-old-translator', $data);
        }

        $new_user = $new_translator->user;
        $new_name = $new_user->name;
        $new_email = $new_user->email;
        $data['user'] = $new_user;

        $this->mailer->send($new_email, $new_name, $subject, 'emails.job-changed-translator-new-translator', $data);
    }
    ...
    public function sendChangedDateNotification($job, $old_time)
    {
        ...
        $data = compact('user', 'job', 'old_time');
    ...
    public function acceptJob($data, $user)
    {
        ...
        if (!Job::isTranslatorAlreadyBooked($jobId, $currentUser->id, $job->due)) {
            ...
            // Set response for successful accept job
            ...
            // Set response for job accept fail
            $response = [
                'status' => 'fail',
    ...
    {
        $translatorType = $user->userMeta->translator_type;
        $jobType = $translatorType === 'professional' ? 'paid' : ($translatorType === 'rwstranslator' ? 'rws' : 'unpaid');
        $languages = $user->userLanguages->pluck('lang_id')->all();
        $gender = $user->userMeta->gender;
        $translatorLevel = $user->userMeta->translator_level;

        $jobs = Job::getJobs($user->id, $jobType, 'pending', $languages, $gender, $translatorLevel);

        foreach ($jobs as $key => $job) {
            $isSpecificJob = Job::assignedToPaticularTranslator($user->id, $job->id) === 'SpecificJob';
            $canAcceptJob = Job::checkParticularJob($user->id, $job) !== 'userCanNotAcceptJob';
            $isPhysicalJob = $job->customer_physical_type === 'yes';
            $hasNoPhone = $job->customer_phone_type === 'no' || $job->customer_phone_type === '';

            if ($isSpecificJob && !$canAcceptJob) {
                unset($jobs[$key]);
            } else if ($isPhysicalJob && $hasNoPhone && !Job::checkTowns($job->user_id, $user->id)) {
                unset($jobs[$key]);
            }
        }
    ...
    public function getAll(Request $request, $limit = null)
    {
        ...
        if ($cuser && $cuser->user_type == env('SUPERADMIN_ROLE_ID')) {
            ...
            foreach ($filterDate as $param => $field) {
                if (isset($requestData[$param]) && $requestData[$param] !== '') {
                    $allJobs->where($field, '>=', $requestData[$param]);
                }
            }
            ...
            $physical = $requestData['physical'] ?? null;
            $phone = $requestData['phone'] ?? null;

            if ($physical !== null) {
                $allJobs->where('customer_physical_type', $physical)
                    ->where('ignore_physical', 0);
            }

            if ($phone !== null) {
                $allJobs->where('customer_phone_type', $phone);
        ...
        foreach ($filters as $param => $field) {
            if (isset($requestData[$param]) && $requestData[$param] !== '') {
                $allJobs->whereIn($field, (array) $requestData[$param]);
            }
        }
